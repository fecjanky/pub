...
\begin{abstract}
    \boldmath
    The handling of connectionless networking protocol messages that has low memory footprint could be implemented
    effectively by using FPGAs.
    When lower level protocols are realized in such way, they will provide fast and predictable service
    characteristics for
    their upper layer protocols. The variance of frame or packet handling delay can easily go out of control for
    software-based protocol implementations. This is due to various factors including the scheduling system of the
    operating
    system, the dynamics of memory handling, among others. These can be eliminated when using hardware-based
    implementations, where data unit handling is counted in clock cycles.
    ...
    The ISO OSI Basic Reference Model for communication is a remarkable guideline as a design basis for setting the
    ...
    handling
    PDUs (Protocol Data Unites) and IDUs (Interface Data Units). Beside these features it also provides solutions
    for other
    ...
    performed on the VHDL description itself. The live tests were carried out on the fully implemented application of ARP
    and Ethernet, which was interconnected with network and transport layer hardware and software products from 3rd party
    vendors.
\end{abstract}
...
The aim of this paper was to present the design and implementation of a generalized framework for FPFA-based networking
...
it is easy and effective to implement protocols with its support.
...
the board with various devices. \cite{Williams_Web_Workload_Characterization_10_Years}

\section{Acknowledgement}
I would like to thank to Aitia International Inc. for supplying the necessary hardware resources for this work and also
to Gabor Krodi for his invaluable feedback and guidance, which was essential for completing this work.
