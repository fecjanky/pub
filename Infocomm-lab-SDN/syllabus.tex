...
%2. Saját topológia
%
%    Az egyszerû hálózati topológia után saját topológiát fogunk létrehozni, ahol a linkek paramétereit is állítani kell. A Mininetet a tc %segítségével tudja emulálni ezeket a paramétereket, de ehhez speciális paraméterezéssel kell indítani (--link=tc).
%        Hozd létre a mérévezető által megadott hálózati topológiát Mininetben!
%        Állítsd be a megfelelő link sávszélességeket és késleltetéseket! (segítség: https://github.com/mininet/mininet/wiki/%Introduction-to-Mininet, http://mininet.org/api/hierarchy.html vagy a Mininet forráskódok)
%        Az alapértelmezett kontroller helyett használd a POX kontrollert (remote controller) a localhost 6633-as portján futtatva (ez a %default)! Használd a forwarding.l2_learning modult!
%            segítség: egy külön terminálból a pox könyvtárba lépve a következő parancsot használd: ./pox.py forwarding.l2_learning %samples.pretty_log 
%        Minden hoszt párra végezzél throughput méréseket az alapértelmezett TCP használatával! Az eredményeket rögzítsd a jegyzőkönyvben!
%        Minden hoszt párra mérd ki az RTT-t! Az eredményeket rögzítsd a jegyzőkönyvben! 
%
%3. POX kontroller alkalmazások
%
%    Ebben a feladatban három különböző POX alkalmazást kell összehasonlítani az előző feladatban létrehozott topológián mûködtetve és rá %kell jönni a mûködésbeli különbségekre. Mindhárom alkalmazás mellett el kell végezni a megfelelő vizsgálatokat, hogy kiderítsétek %hogyan mûködik a hálózat az egyes esetekben. Az alkalmazások a következőképpen indíthatók:
%        ./pox.py samples.feladat1 samples.pretty_log
%        ./pox.py samples.feladat2 samples.pretty_log
%        ./pox.py --verbose samples.feladat3 samples.pretty_log openflow.discovery 
%
%    Teszteld a hálózat mûködését!
%        Egy tesztelési lehetőség: végezzünk ping tesztet a H1 és H4 hosztok között és közben
%            rögzítsük a kontroll csatorna forgalmát wireshark segítségével
%            vizsgáljuk az egyes switch-ek flow bejegyzéseit
%            (kövessük a POX alkalmazások log üzeneteit) 
%        Milyen mûködést valósítank meg az egyes alkalmazások, mik a lényegi, mûködésbeli különbségek?
%        Segítség feladat3-hoz:
%            Mininet alatt töröljük a statikus IP címeket a hosztokról
%                pl. H1-re a Mininet terminálban: h1 ip addr del 10.0.0.1/8 dev h1-eth0 
%            helyette kérjünk DHCP-vel címet
%                pl. H1-re a Mininet terminálban: h1 dhclient -v h1-eth0 
%
