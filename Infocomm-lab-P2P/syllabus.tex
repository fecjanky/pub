...
\begin{itemize}
   \item When was the appearance of the P2P paradigm dated? What were the motivators behind it? What are the main categories of P2P applications? 
   \item What is the most popular P2P file sharing technology currently? What are the components of the system? What is the principle of its operation?
   \item What is the problem of the ISPs with P2P applications (beside copyright issues)? What solutions had appeared to overcome those problems?
...
\begin{itemize}
    ...
    \item \href{https://qosip.tmit.bme.hu/foswiki/pub/Meres/P2PAlkalmazasokMeresiSegedlet/p2pMeresTopo.py.txt}
       {p2pMeresTopo.py.txt}: Mininet topology for the P2P measurements, run by executing: `sudo python p2pMeresTopo.py'
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item Start a terminal and disable Network Manager with executing `sudo service network-manager stop' command. Then execute `sudo dhclient eth0' to restore the NAT IP address
		\item Start the controller using the command `pox/pox.py p2p' command (pox is under /home/p2p/pox/). The INFO logs can be filtered by passing `log.level --WARNING' startup argument, but in this case the own log messages has to be logged at least on log level warning
		\item Start Mininet with command `sudo python p2pMeresTopo.py' (the mininet topology script can be found under directory /home/p2p/)
	\end{enumerate}
	\item Mininet setup
	\begin{enumerate}
		\item Verify connections using command 'pingall' from the Mininet shell
		\item From the mininet shell start xterm windows on the selected hosts (e.g `xterm Leecher1 Leecher2 Leecher3 Seed')
		\item Set the home directory on each host (Leecher1-3,Seed) by executing the command `export HOME=/home/p2p/<HOST NAME>' from the corresponding xterm window
	\end{enumerate}
\end{enumerate}
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item when creating the torrent file specify the Tracker as ``http://<IP address>:<Tracker port>/announce"
	...
	\begin{enumerate}
		...
		\item start the video client: Connect to the VLC network stream using VLC media player on ``http://<Server IP address>:8080"
