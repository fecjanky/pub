...
\begin{abstract}
    ...
    effectively by using Field Programmable Gate Arrays (FPGA).
    ...
    performed on the VHSIC Hardware Description Language (VHDL) description itself. The live tests were carried out on
    the fully implemented application of Address Resolution Protocol (ARP)
...
The motivation for this work is to design and implement a framework in VHSIC Hardware Description Language (VHDL) that
enables rapid prototyping of
...
Information (ICI)  and Service Data Unit (SDU). Given these, it is beneficial to have a generic component that can be
reused
...
Fig.~\ref{fig:system_sketch} shows the high level design of the Protocol Layer module. The Protocol Under
Implementation (PUI) and Protocol Independent Part (PIP) part is
...
On Fig.~\ref{fig:eth_sim} the simulation waveforms of the Ethernet module can be seen. It illustrates a Media Access
Control (MAC) address
...
Validation of the Address Resolution Protocol (ARP)~\cite{RFC0826-ARP} module -- that is implemented through the
\emph{ProtoLayer} module
-- was performed by interconnecting the device with a Layer 2 switch and operating it on a Local Area Network (LAN)
segment. The module was
