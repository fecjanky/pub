...
Given the parameters above and \eqref{eq:tframe} and \eqref{eq:tbit}, the frame time is 90/\SI{15.625}{\mega\hertz} = \SI{5.76}{\micro\second}. Since every
...
calculating the skew of the clock periods under a frame time 

\begin{equation}\label{eq:clkerr}
CLK_{skew} = \lfloor  \lvert \frac{T_{frame}}{T_{sys}} - T_{frame} \times f_{sys} \times E_{oscillator} \rvert \rfloor
\end{equation}
...
Where T\textsubscript{frame} is \SI{11.52}{\micro\second} and typical values for the system clock are
f\textsubscript{sys} is \SI{125}{\mega\hertz} and E\textsubscript{oscillator} is $\pm$ 50 ppm. Substituting these values
into \eqref{eq:clkerr} gives that it is $\lfloor 0.072 \rfloor = 0$ clock periods.
...
time-stamp is 64 bits wide -- also using the NTP binary fractional representation -- 
with N=64 then the theoretical accuracy is \SI{232.83}{\pico\second}.

%%%%%%%%%%%% mit jelent az a stabilitás az extrernal esetben? Hogyan számítódik?
%%%%%%%%%%%% hogyan számítódik ez a "theoretical accuracy"?
%%%%%%%%%%%% kellene ide két képlet (legalább)
