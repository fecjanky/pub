...
\begin{algorithmic} % enter the algorithmic environment
    ...
    \FOR{Each rising edge of system clock}
        \IF{Received valid time-stamp from Master}
...
%From a practical point of view doubling the clock frequency would have been a small gain in accuracy marginally.
...
As for the internal synchronization part, it is by design has accuracy and precision in the magnitude of \SI{1}{\nano\second} -- see Section~\ref{sec:Internal-Impl} for details.% accurate as it is a message synchronous bus system with no
...
%calculating the skew of the clock periods under a time-stamp update period: 
...
%Where T\textsubscript{frame} (frame time over the internal synchronization bus) is \SI{5.76}{\micro\second}, and the typical values of the system clock parameters are
...
%Based on the considerations above, the accuracy simply equals to the resolution of the digital time-stamp given by \eqref{eq:resolution}.
%\begin{equation}\label{eq:resolution}
%Resolution = 2^{-N/2} \SI{}{\second}
%\end{equation}

%As it was described in Section~\ref{sec:Internal-Impl}, the
%time-stamp is 64 bits wide -- since it uses the NTP binary fractional representation. 
